...
func loadOfficeData(req Data, invoiceID string) OfficeTemplateData {
...
func getMonthsN(req empl.Data, num int, now time.Time) emplDto.Months {
...
func loadTodayAndPastInvoices(req empl.Data, numPastInvoices int, today time.Time) dto.EmployeesHistoricReport {
...
func (h handler) Home(vars map[string]string, req *http.Request) interface{} {
	client, err := NewCodaData(h.config.BaseUri, h.config.ApiTokenOf, h.config.DocIdOf)
	if err != nil {
		return Page{Body: template.HTML(err.Error())}
	}
...
func (h handler) ShowInvoiceData(vars map[string]string, req *http.Request) interface{} {
	officeClient, offErr := NewCodaData(h.config.BaseUri, h.config.ApiTokenOf, h.config.DocIdOf)
	if offErr != nil {
		return Page{Body: template.HTML(offErr.Error())}
	}
	officeData := loadOfficeData(officeClient, vars["invoice"])

	emplClient, emplErr := empl.NewCodaData(h.config.BaseUri, h.config.ApiTokenEm, h.config.DocIdEm)
	if emplErr != nil {
		return Page{Body: template.HTML(emplErr.Error())}
	}
