	...
	for ii, item := range shallow {
		if rels.LegalEntity {
			for vi, v := range item.LegalEntity.Values {
				...
				if ok {
					shallow[ii].LegalEntity.Values[vi].Data = &data
				}
			}
		}
		if rels.Location {
			for vi, v := range item.Location.Values {
				if v.Data != nil {
					continue
				}

				data, ok := _locationMap[RowID(v.RowID)]

				if ok {
					shallow[ii].Location.Values[vi].Data = &data
				}
			}
		}
		if rels.BankTariffs {
			for vi, v := range item.BankTariff.Values {
				if v.Data != nil {
					continue
				}

				data, ok := _bankTariffsMap[RowID(v.RowID)]

				if ok {
					shallow[ii].BankTariff.Values[vi].Data = &data
				}
			}
		}
		if rels.BankDetails {
	...
	for ii, item := range shallow {
		if rels.Invoice {
			for vi, v := range item.Invoice.Values {
				...
				data, ok := _invoiceMap[RowID(v.RowID)]
...
func (doc *CodaDocument) LoadRelationsPatentCompensation(ctx context.Context, shallow map[RowID]PatentCompensation, rels Tables) (err error) {
	...
	for ii, item := range shallow {
		if rels.Invoice {
			for vi, v := range item.PaymentInvoice.Values {
				if v.Data != nil {
					continue
				}
				...
				if ok {
					shallow[ii].PaymentInvoice.Values[vi].Data = &data
				}
			}
		}
		if rels.EmployeePatents {
			for vi, v := range item.EmployeePatentRef.Values {
				if v.Data != nil {
					continue
				}

				data, ok := _employeePatentsMap[RowID(v.RowID)]

				if ok {
					shallow[ii].EmployeePatentRef.Values[vi].Data = &data
				}
			}
		}
	}
...
func (doc *CodaDocument) LoadRelationsEmployeePatents(ctx context.Context, shallow map[RowID]EmployeePatents, rels Tables) (err error) {
	...
	for ii, item := range shallow {
		...
		if rels.TaxYears {
			for vi, v := range item.Year.Values {
				if v.Data != nil {
					continue
				}

				data, ok := _taxYearsMap[RowID(v.RowID)]

				if ok {
					shallow[ii].Year.Values[vi].Data = &data
				}
			}
		}
	}
...
func (doc *CodaDocument) LoadRelationsFullPayrollReport(ctx context.Context, shallow map[RowID]FullPayrollReport, rels Tables) (err error) {
	...
	for ii, item := range shallow {
		if rels.Invoice {
			for vi, v := range item.InvoiceRef.Values {
				if v.Data != nil {
					continue
				}
				...
				if ok {
					shallow[ii].InvoiceRef.Values[vi].Data = &data
				}
			}
		}
	}
...
func (doc *CodaDocument) LoadRelationsBankDetails(ctx context.Context, shallow map[RowID]BankDetails, rels Tables) (err error) {
	...
	for ii, item := range shallow {
		...
		if rels.BeneficiaryBank {
			for vi, v := range item.BeneficiaryBank.Values {
				if v.Data != nil {
					continue
				}

				data, ok := _beneficiaryBankMap[RowID(v.RowID)]
	...
	for ii, item := range shallow {
		if rels.PerDayPolicies {
			for vi, v := range item.Type.Values {
				...
				data, ok := _perDayPoliciesMap[RowID(v.RowID)]
		...
		if rels.Invoice {
			...
			for vi, v := range item.CalculationInvoice.Values {
				if v.Data != nil {
					continue
				}
				...
				if ok {
					shallow[ii].CalculationInvoice.Values[vi].Data = &data
				}
			}
		}
		if rels.Months {
			for vi, v := range item.SalaryPeriod.Values {
				if v.Data != nil {
					continue
				}
				...
				if ok {
					shallow[ii].SalaryPeriod.Values[vi].Data = &data
				}
			}
			for vi, v := range item.WorkPeriod.Values {
				if v.Data != nil {
					continue
				}
				...
				if ok {
					shallow[ii].WorkPeriod.Values[vi].Data = &data
				}
			}
		}
	}
...
func (doc *CodaDocument) LoadRelationsPerDayPolicies(ctx context.Context, shallow map[RowID]PerDayPolicies, rels Tables) (err error) {
	...
	for ii, item := range shallow {
		if rels.EntryType {
			for vi, v := range item.EntryType.Values {
				if v.Data != nil {
					continue
				}
				...
				if ok {
					shallow[ii].EntryType.Values[vi].Data = &data
				}
			}
		}
	}
...
func (doc *CodaDocument) LoadRelationsTemplateEntriesTypeFiltering(ctx context.Context, shallow map[RowID]TemplateEntriesTypeFiltering, rels Tables) (err error) {
	...
	for ii, item := range shallow {
		if rels.EntryType {
			...
			for vi, v := range item.FilteringType.Values {
				if v.Data != nil {
					continue
				}
				...
				if ok {
					shallow[ii].FilteringType.Values[vi].Data = &data
				}
			}
		}
	}
